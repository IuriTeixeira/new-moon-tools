            ...
            <div class="container">
              <div class="content">
                ...
                <h3>
                  ...
                  ></span>
                  {{ pluralize }} required to go from Class
                  {{ options.startClass }} Rank {{ options.startRank }} to Class
                  {{ options.endClass }} Rank {{ options.endRank }}
                </h3>
              </div>
            </div>
          </div>
...
export default {
  ...
  computed: {
    books() {
      let i = 0;
      let startingExpertise =
        this.options.startClass * 1000 + this.options.startRank * 100;
      let currentExpertise = startingExpertise;
      let endingExpertise =
        this.options.endClass * 1000 + this.options.endRank * 100;
      let modifier = this.modifier;

      for (i = 0; currentExpertise < endingExpertise; i++) {
        let raw = this.roundToThousandth(
          150 /
            (this.expertiseClass(currentExpertise) + 1) /
            (this.expertiseRank(currentExpertise) + 1)
        );
    ...
    modifier() {
      var r =
        1 + parseInt(this.options.incense) + this.options.gearXpBoost / 100;
      return r;
    },
    pluralize() {
      if (this.books == 1) return "Textbook";
      return "Textbooks";
    },
  },
  methods: {
